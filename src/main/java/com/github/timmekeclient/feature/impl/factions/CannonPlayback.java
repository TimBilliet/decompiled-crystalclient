...
import com.github.timmekeclient.mixin.accessor.net.minecraft.entity.item.MixinEntityFallingBlock;
import com.github.timmekeclient.util.enums.AnchorRegion;
import com.github.timmekeclient.util.objects.ModulePosition;
import com.github.timmekeclient.util.objects.cannonplayback.PlaybackEntityFallingBlock;
import com.github.timmekeclient.util.objects.cannonplayback.PlaybackEntityTNTPrimed;
...
public class CannonPlayback extends HudModuleBackground implements IRegistrable {
    ...
    public KeyBinding record = new KeyBinding("timmekeclient.key.start_recording", 0, "Timmeke_ Client - Cannon Playback");

    @Keybind(label = "Reset Recording")
    public KeyBinding reset = new KeyBinding("timmekeclient.key.reset_recording", 0, "Timmeke_ Client - Cannon Playback");

    @Keybind(label = "Previous Frame")
    public KeyBinding prevFrame = new KeyBinding("timmekeclient.key.previous_frame", 0, "Timmeke_ Client - Cannon Playback");

    @Keybind(label = "Next Frame")
    public KeyBinding nextFrame = new KeyBinding("timmekeclient.key.next_frame", 0, "Timmeke_ Client - Cannon Playback");
