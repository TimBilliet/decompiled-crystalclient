...
public class FloatFinder extends Module implements IRegistrable {
    ...
    public KeyBinding selectAdjustBlock = new KeyBinding("timmekeclient.key.select_barrel_block", 0, "Timmeke_ Client - Float Finder");

    @HoverOverlay({"Required for flipping barrels"})
    @Keybind(label = "Select power block")
    public KeyBinding selectPowerBlock = new KeyBinding("timmekeclient.key.select_power_block", 0, "Timmeke_ Client - Float Finder");

    @Keybind(label = "Calculate float location")
    public KeyBinding calcFloatLocation = new KeyBinding("timmekeclient.key.calc_float_location", 0, "Timmeke_ Client - Float Finder");
