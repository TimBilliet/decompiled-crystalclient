...
public class SneezeEmote extends Emote {
  public SneezeEmote(String name, int duration, boolean looping, String sound) {
  ...
  public void progressAnimation(EntityLivingBase entity, BOBJArmature armature, AnimatorEmoticonsController animator, int tick, float partial) {
    ...
    if (tick == Time.toTicks(121) - 1)
      for (int i = 0; i < 10; i++) {
        ...
        entity.worldObj.spawnParticle(EnumParticleTypes.CLOUD, result.x, result.y, result.z, rand(0.05F), -0.02500000037252903D, rand(0.05F), new int[0]);
      }  
  }
}


/* Location:              C:\Users\Tim\AppData\Roaming\.minecraft\mods\temp\Crystal_Client-1.1.16-projectassfucker_1.jar!\mchorse\emoticons\common\emotes\SneezeEmote.class
